...
import com.example.pizza_service.services.OrderService;
import com.example.pizza_service.services.PizzaService;
import com.example.pizza_service.services.UserService;
...
public class OrderController {
    ...
    private Order mapToOrder(OrderDto orderDto){
        ...
        order.setUser(user);
        order.setPizzas(dtoPizzaMapToList(orderDto.getPizzaId()));
        return order;
    }
    private List<Pizza> dtoPizzaMapToList(List<Integer> pizzaId){
        List<Pizza> pizzas = new ArrayList<>();
        for(int i = 0;i< pizzaId.size();i++){
            Pizza pizza = checkForPresenceAndGetPizza(pizzaId.get(i));
            pizzas.add(pizza);
        }
        return pizzas;
    }
    private void updateOrders(Order editable, OrderDto changes) {
        ...
        if(nonNull(changes.getPizzaId())){
            List<Pizza> pizzas = dtoPizzaMapToList(changes.getPizzaId());
            editable.setPizzas(pizzas);
        }
    }
